...
Schema construction validates defaults and rejects unsupported metadata. A malformed declaration raises `DeclarationError`, a `TypeError` subclass.
...
- An optional injected context, configured by name, as the first required positional parameter.
...
The declaration retains these markers in `Parameter.metadata` and removes them from the validation annotation.
...
- `Tool.parameters`: ordered `Parameter` records with the original annotation, validation annotation, default, description, scalar type, choices, repeated/nullable flags, and application metadata.
- `Parameter.required`: whether the signature has no default.
- `Tool.description` and `Tool.summary`: full docstring and first line, or an empty string.
- `Tool.argument_model(include_hidden=False, name=None, descriptions=None)`: the cached Pydantic model for that projection.
